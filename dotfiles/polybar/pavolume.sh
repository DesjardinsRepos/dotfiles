...
function waitForSinkConnect {
	# Basically does the same as this snippet but actually kills the pactl process
	# after "card" is read. Thanks to u/oh5nxo for this :)
	#
	# pactl subscribe | grep --line-buffered card | while read -r UNUSED_LINE; do
    # 	  break								
	# done;
	# echo $(getIndex)
	...
	coproc pactl subscribe
	if grep -q card <&${COPROC[0]}
	then
	    echo $(getIndex)
	fi
	kill $COPROC_PID
